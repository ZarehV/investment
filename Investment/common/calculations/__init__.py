...
import ta
from common.journal import input_int_with_default
...
def calculate_recommended_take_profit(symbol, purchase_price, stop_loss, risk_reward_ratio=2):
    if purchase_price > stop_loss:
        ...
        print(
            f"Purchase price is of {purchase_price} lower that stop lost of {stop_loss}, last closing price for {symbol} was {last_closing_price}"
        )
        recommended_take_profit = get_recommended_take_profit(
            symbol, purchase_price, last_closing_price
        )
...
def calculate_porfit_loss(purchase_price, exit_price, position_size):
    ...
    return final_position_dollars - initial_postion_dollars, (
        final_position_dollars - initial_postion_dollars
    ) / initial_postion_dollars
