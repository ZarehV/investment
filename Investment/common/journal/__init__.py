...
def add_details_to_new_position(new_investment):
    ...
    while True:
        investment_account = input(
            "Provide Account Name TradeStation Sim (TS) or TradeStation Live (TSL) or Robinhood Investment (RI) or Robinhood Retirement (RR):"
        )
...
def close_position():
    ...
    if len(investments_pd[investments_pd["key"] == key]) > 0:
        ...
        final_PL = (
            record["position_size"] * filled_price
            - record["position_size"] * record["purchase_price"]
        )
        investments_pd.loc[index, "final_PL"] = final_PL

        final_pct_revenue = round(
            final_PL / (record["position_size"] * record["purchase_price"]), 5
        )
