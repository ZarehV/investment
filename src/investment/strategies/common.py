...
def get_closing_data(symbols, start_date, interval, end_date=None):
    ...
    data = yf.download(symbols, start=start_date.strftime('%Y-%m-%d'), end=end_date, interval=interval)['Close']
    return data
...
def get_assets_by_momentum(symbols, returns_pd, periods, total_periods=12):
    ...
    return momentum_df.sort_values(by='momentum_score', ascending=False).reset_index().rename(
...
def get_top_assets(momentum_df, bitcoin_symbol, top_n=4):
    ...
    symbols = momentum_df.sort_values(
        by='momentum_score',
        ascending=False
    ).head(top_n)['symbol'].unique()
    ...
    if is_bitcoin_in_top:
        ...
        return momentum_df.sort_values(by='momentum_score', ascending=False).head(top_n), is_bitcoin_in_top

    return momentum_df.sort_values(by='momentum_score', ascending=False).head(top_n - 1), is_bitcoin_in_top
